...
:::{.subexample}
Para comparar se dois valores são iguais,  usamos a operação `=` cujo valor é um [valor lógico](https://docs.gap-system.org/doc/ref/chap20_mj.html#X787B4AB77A2F5E14) ou seja `true` ou `false`. As operações com valores lógicos são `not`, `and` e `or` que funcionam exatamente como os respetivos conetivos na lógica matemática.
```matlab
gap> 2 + 3 = 5;
true
gap> 2 + 3 = 5 or 2 + 3 = 6;
