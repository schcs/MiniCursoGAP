...
3. Um número $n$ chama-se *abundante*, se $\sigma(n)>n$. Escreva uma função `is_abundant( n )` que devolve `true` se o número `n` é  abundante e `false` se não é abundante. 
4. Ache todos os números abundantes entre `1` e `1000`.
</div>
